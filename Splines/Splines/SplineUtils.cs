...
/// <summary>
/// Various utility functions for splines.
/// </summary>
public static class SplineUtils
{
    /// <summary>
    /// Samples a Bernstein polynomial Bézier basis function.
    /// </summary>
    ...
    /// <returns>The sampled value of the Bernstein basis function.</returns>
    [Pure]
    ...
    /// <param name="pCount">The number of control points.</param>
    /// <param name="open">Whether the spline is open or closed.</param>
    /// <returns>An array of knot values.</returns>
    [Pure]
    ...
    /// <summary>
    /// Calculates the number of knots required for a B-spline.
    /// </summary>
    /// <param name="pointCount">The number of control points.</param>
    /// <param name="degree">The degree of the spline.</param>
    /// <returns>The number of knots required.</returns>
    [Pure]
    internal static int BSplineKnotCount(int pointCount, int degree) => degree + pointCount + 1;

    /// <summary>
    /// Calculates a Catmull-Rom knot value.
    /// </summary>
    /// <param name="kPrev">The previous knot value.</param>
    /// <param name="sqDist">The squared distance between points.</param>
    /// <param name="alpha">The alpha parameter for the Catmull-Rom spline.</param>
    /// <param name="isSquaredDist">Whether the distance is squared.</param>
    /// <returns>The calculated knot value.</returns>
    [Pure]
    public static float CalcCatRomKnot(float kPrev, float sqDist, float alpha, bool isSquaredDist)
    {
        return kPrev + CalcCatRomKnot(sqDist, alpha, isSquaredDist).AtLeast(0.00001f);
    }

    /// <summary>
    /// Calculates a Catmull-Rom knot value based on distance.
    /// </summary>
    /// <param name="dist">The distance between points.</param>
    /// <param name="alpha">The alpha parameter for the Catmull-Rom spline.</param>
    /// <param name="isSquaredDist">Whether the distance is squared.</param>
    /// <returns>The calculated knot value.</returns>
    [Pure]
    ...
    /// <summary>
    /// Gets the uniform knots.
    /// </summary>
    /// <param name="unitInterval">Whether to use the unit interval.</param>
    /// <returns>A <see cref="Matrix4x1"/> containing the uniform knots.</returns>
    [Pure]
    private static Matrix4x1 GetUniformKnots(bool unitInterval) => unitInterval ? knotsUniformUnit : knotsUniform;

    /// <summary>
    /// Calculates the Catmull-Rom knots for a set of 2D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="alpha">The alpha parameter for the Catmull-Rom spline.</param>
    /// <param name="unitInterval">Whether to use the unit interval.</param>
    /// <returns>A <see cref="Matrix4x1"/> containing the Catmull-Rom knots.</returns>
    [Pure]
    public static Matrix4x1 CalcCatRomKnots(Vector2Matrix4x1 m, float alpha, bool unitInterval)
    {
        if (alpha == 0) return GetUniformKnots(unitInterval); // uniform catrom

        float sqMag01 = (m.M0 - m.M1).SqrMagnitude();
        float sqMag12 = (m.M1 - m.M2).SqrMagnitude();
        float sqMag23 = (m.M2 - m.M3).SqrMagnitude();
        return CalcCatRomKnots(sqMag01, sqMag12, sqMag23, alpha, unitInterval, isSquaredDist: true);
    }

    /// <summary>
    /// Calculates the Catmull-Rom knots for a set of 3D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="alpha">The alpha parameter for the Catmull-Rom spline.</param>
    /// <param name="unitInterval">Whether to use the unit interval.</param>
    ...
    /// <summary>
    /// Calculates the Catmull-Rom knots for given squared distances.
    /// </summary>
    /// <param name="dist01">The squared distance between the first and second points.</param>
    /// <param name="dist12">The squared distance between the second and third points.</param>
    /// <param name="dist23">The squared distance between the third and fourth points.</param>
    /// <param name="alpha">The alpha parameter for the Catmull-Rom spline.</param>
    /// <param name="unitInterval">Whether to use the unit interval.</param>
    /// <param name="isSquaredDist">Whether the distances are squared.</param>
    /// <returns>A <see cref="Matrix4x1"/> containing the Catmull-Rom knots.</returns>
    [Pure]
    ...
    /// <summary>
    /// Gets the non-uniform Catmull-Rom characteristic matrix for given knots.
    /// </summary>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    /// <returns>A <see cref="Matrix4x4"/> representing the characteristic matrix.</returns>
    [Pure]
...
#pragma warning restore BDX0023
    ...
    /// <summary>
    /// Gets the non-uniform Catmull-Rom characteristic matrix for given knots and control points.
    /// </summary>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    ...
    /// <returns>A <see cref="Vector3"/> representing the characteristic matrix end.</returns>
    [Pure]
    public static Vector3 GetNUCatRomCharMatrixC2End(
        Matrix4x1 knots,
        Vector3 p0,
        Vector3 p1,
        Vector3 p2,
    ...
    /// <summary>
    /// Gets the non-uniform Catmull-Rom characteristic matrix for the unit interval.
    /// </summary>
...
#pragma warning restore BDX0023
    ...
    /// <summary>
    /// Calculates the Catmull-Rom curve for 1D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    /// <returns>A <see cref="Polynomial1D"/> representing the Catmull-Rom curve.</returns>
    [Pure]
    internal static Polynomial1D CalculateCatRomCurve(Matrix4x1 m, Matrix4x1 knots)
        => new(GetNUCatRomCharMatrix(knots).MultiplyColumnVector(m));

    /// <summary>
    /// Calculates the Catmull-Rom curve for 2D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    /// <returns>A <see cref="Polynomial2D"/> representing the Catmull-Rom curve.</returns>
    [Pure]
    internal static Polynomial2D CalculateCatRomCurve(Vector2Matrix4x1 m, Matrix4x1 knots)
        => new(GetNUCatRomCharMatrix(knots).MultiplyColumnVector(m));

    /// <summary>
    /// Calculates the Catmull-Rom curve for 3D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    /// <returns>A <see cref="Polynomial3D"/> representing the Catmull-Rom curve.</returns>
    [Pure]
    internal static Polynomial3D CalculateCatRomCurve(Vector3Matrix4x1 m, Matrix4x1 knots)
        => new(GetNUCatRomCharMatrix(knots).MultiplyColumnVector(m));

    /// <summary>
    /// Calculates the Catmull-Rom curve for 4D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="knots">The knots of the Catmull-Rom spline.</param>
    /// <returns>A <see cref="Polynomial4D"/> representing the Catmull-Rom curve.</returns>
    [Pure]
    internal static Polynomial4D CalculateCatRomCurve(Vector4Matrix4x1 m, Matrix4x1 knots)
        => new(GetNUCatRomCharMatrix(knots).MultiplyColumnVector(m));

    /// <summary>
    /// Calculates the Hermite curve for 3D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="k0">The first knot value.</param>
    /// <param name="k1">The second knot value.</param>
    /// <returns>A <see cref="Polynomial3D"/> representing the Hermite curve.</returns>
    [Pure]
    internal static Polynomial3D CalculateHermiteCurve(Vector3Matrix4x1 m, float k0, float k1)
        => new(GetNUHermiteCharMatrix(k0, k1).MultiplyColumnVector(m));

    /// <summary>
    /// Calculates the Hermite curve for 2D control points.
    /// </summary>
    /// <param name="m">The matrix of control points.</param>
    /// <param name="k0">The first knot value.</param>
    /// <param name="k1">The second knot value.</param>
    /// <returns>A <see cref="Polynomial2D"/> representing the Hermite curve.</returns>
    [Pure]
    internal static Polynomial2D CalculateHermiteCurve(Vector2Matrix4x1 m, float k0, float k1)
        => new(GetNUHermiteCharMatrix(k0, k1).MultiplyColumnVector(m));

    /// <summary>
    /// Gets the non-uniform Hermite characteristic matrix.
    /// </summary>
    /// <param name="k0">The first knot value.</param>
    /// <param name="k1">The second knot value.</param>
    /// <returns>A <see cref="Matrix4x4"/> representing the characteristic matrix.</returns>
