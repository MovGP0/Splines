...
public static class NURBS1DExtensions
{
    /// <summary>
    ...
    /// <returns>A new NURBS1D object representing the smoothed curve.</returns>
    [Pure]
    public static NURBS1D Smooth(this NURBS1D nurbs, int iterations = 1)
    {
        if (iterations < 1)
        {
        ...
        float[] smoothedPoints = (float[])nurbs.Points.Clone();

        for (int iter = 0; iter < iterations; iter++)
        {
            float[] newPoints = new float[nurbs.PointCount];
        ...
        return new NURBS1D(smoothedPoints, clonedKnots, clonedWeights, nurbs.Degree);
    }
}
