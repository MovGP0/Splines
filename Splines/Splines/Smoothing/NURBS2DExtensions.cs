...
public static class NURBS2DExtensions
{
    /// <summary>
    ...
    /// <returns>A new NURBS2D object representing the smoothed curve.</returns>
    [Pure]
    public static NURBS2D Smooth(this NURBS2D nurbs, int iterations = 1)
    {
        if (iterations < 1)
        {
        ...
        Vector2[] smoothedPoints = (Vector2[])nurbs.Points.Clone();

        for (int iter = 0; iter < iterations; iter++)
        {
            Vector2[] newPoints = new Vector2[nurbs.PointCount];
        ...
        return new NURBS2D(smoothedPoints, clonedKnots, clonedWeights, nurbs.Degree);
    }
}
