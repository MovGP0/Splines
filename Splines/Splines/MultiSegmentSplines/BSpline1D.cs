...
public sealed class BSpline1D
{
    ...
    public BSpline1D(float[] points, float[] knots, int degree = 3)
    {
        Points = points;
        Knots = knots;
        Degree = degree;
        _evalBuffer = new float[degree + 1];
    ...
    public BSpline1D(float[] points, int degree = 3, bool open = false)
    {
        Points = points;
    ...
    public BSpline1D Differentiate()
    {
        float[] dKnots = new float[KnotCount];
        Knots.CopyTo(dKnots, 0);

        // one point less
        float[] dPts = new float[PointCount - 1];
        for (int i = 0; i < dPts.Length; i++) {
            float num = Points[i + 1] - Points[i];
            float den = Knots[i + Degree + 1] - Knots[i + 1];
            float scale = den == 0 ? 0 : Degree / den;
            dPts[i] = num * scale;
        }

        return new BSpline1D(dPts, dKnots, Degree - 1);
    }
    ...
    public float GetSegmentPoint(int segment, float t)
    {
    ...
    public float GetPointByKnotIndex(int i)
    {
    ...
    public float GetPointByKnotValue(float u)
    {
        int i = 0;
    ...
    public float Eval(int k, float u)
    {
        ...
        for (int r = 1; r < Degree + 1; r++)
        {
            for (int j = Degree; j > r - 1; j--)
            {
                ...
                _evalBuffer[j] = Mathf.LerpUnclamped(_evalBuffer[j - 1], _evalBuffer[j], alpha);
            }
        }
