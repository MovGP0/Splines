...
public sealed class BSpline2D
{
    ...
    public BSpline2D(Vector2[] points, float[] knots, int degree = 3)
    {
        ...
        Knots = knots;
        Degree = degree;
        _evalBuffer = new Vector2[degree + 1];
    ...
    public Vector2 Eval(int k, float u)
    {
        ...
        for (int r = 1; r < Degree + 1; r++)
        {
            for (int j = Degree; j > r - 1; j--)
            {
                ...
                _evalBuffer[j] = _evalBuffer[j - 1].LerpUnclamped(_evalBuffer[j], alpha);
            }
        }
