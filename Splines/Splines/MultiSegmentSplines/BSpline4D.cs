...
public sealed class BSpline4D
{
    ...
    public BSpline4D(Vector4[] points, float[] knots, int degree = 3)
    {
        Points = points;
        Knots = knots;
        Degree = degree;
        _evalBuffer = new Vector4[degree + 1];
    ...
    public BSpline4D(Vector4[] points, int degree = 3, bool open = false)
    {
        Points = points;
    ...
    public BSpline4D Differentiate()
    {
        float[] dKnots = new float[KnotCount];
        Knots.CopyTo(dKnots, 0);

        // one point less
        Vector4[] dPts = new Vector4[PointCount - 1];
        for (int i = 0; i < dPts.Length; i++) {
            Vector4 num = Points[i + 1] - Points[i];
            float den = Knots[i + Degree + 1] - Knots[i + 1];
            float scale = den == 0 ? 0 : Degree / den;
            dPts[i] = num * scale;
        }

        return new BSpline4D(dPts, dKnots, Degree - 1);
    }
    ...
    public Vector4 GetSegmentPoint(int segment, float t)
    {
    ...
    public Vector4 GetPointByKnotIndex(int i)
    {
    ...
    public Vector4 GetPointByKnotValue(float u)
    {
        int i = 0;
    ...
    public Vector4 Eval(int k, float u)
    {
        ...
        for (int r = 1; r < Degree + 1; r++)
        {
            for (int j = Degree; j > r - 1; j--)
            {
                ...
                _evalBuffer[j] = Vector4.Lerp(_evalBuffer[j - 1], _evalBuffer[j], alpha);
            }
        }
    ...
    public Vector4 GetPointKnotAndWeightByLocalSpan(int point, float t)
    {
        ...
        return new Vector4(u, v, 0, 0); // Adding 0 for the Z and W-coordinates
    }
}
