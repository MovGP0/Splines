...
/// <summary>
/// Represents a Catmull-Rom spline in 1D space, with support for different endpoint modes and alpha values.
/// </summary>
[Serializable]
public sealed partial class CatRom1DSpline
{
    /// <summary>
    ...
    /// <inheritdoc cref="NUCatRomCubic1D.Alpha"/>
    public float Alpha
    {
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public CatRom1DSpline(
        IReadOnlyCollection<float> points,
        ...
        EndpointMode endpointMode = EndpointMode.None)
    {
        ...
        Nodes = points.Zip(knots, (p, k) => new Node1 { Position = p, Knot = k }).ToList();
        _isDirty = true;
        ...
        EndpointMode endpointMode = EndpointMode.None)
    {
        ...
        Nodes = points.Select(p => new Node1 { Position = p }).ToList();
        _alpha = alpha;
        _isDirty = true;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetSecondDerivative(float u)
    {
        u = ReadyAndClampU(u);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetThirdDerivative(float u)
    {
    ...
    public void ReadyKnotsAndCoefficients()
    {
        if (_isDirty)
        {
            ...
            for (int i = 0; i < ControlPointCount - 1; i++)
            {
                ...
                Matrix4x1 pts = new(GetControlPoint(i - 1), n.Position, GetControlPoint(i + 1), GetControlPoint(i + 2));
    ...
    public void RecalculateKnots()
    {
        ...
        { // non-uniform
            ...
            {
                float sqDist = (Nodes[i - 1].Position - Nodes[i].Position) * (Nodes[i - 1].Position - Nodes[i].Position);
    ...
    public float GetControlPoint(int index)
    {
        ...
        {
            return Mathf.LerpUnclamped(Nodes[1].Position, Nodes[0].Position, 2);
        }

        if (index == ControlPointCount)
        {
            return Mathf.LerpUnclamped(Nodes[ControlPointCount - 2].Position, Nodes[ControlPointCount - 1].Position, 2);
        }
    ...
    public void SetControlPoint(int index, float position)
    {
        _isDirty = true;
        Node1 n = Nodes[index];
        n.Position = position;
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetPoint(int curve, float t) => GetPointInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the derivative with respect to <c>u</c> along a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    /// <param name="t">The fraction along this segment from 0 to 1</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetDerivative(int curve, float t) => GetDerivativeInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the second derivative with respect to <c>u</c> along a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    /// <param name="t">The fraction along this segment from 0 to 1</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetSecondDerivative(int curve, float t) => GetSecondDerivativeInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the third derivative with respect to <c>u</c> of a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    /// <param name="t">The fraction along this segment from 0 to 1</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public float GetThirdDerivative(int curve, float t) => GetThirdDerivativeInternal(curve, RangeCheckAndGetU(curve, t));
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private float GetPointInternal(int curve, float u) => Nodes[curve].EvalPoint(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private float GetDerivativeInternal(int curve, float u) => Nodes[curve].EvalDerivative(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private float GetSecondDerivativeInternal(int curve, float u) => Nodes[curve].EvalSecondDerivative(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private float GetThirdDerivativeInternal(int curve, float u = 0) => Nodes[curve].EvalThirdDerivative(u);
