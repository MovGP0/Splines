...
/// <summary>
/// Represents a Non-Uniform Rational B-Spline (NURBS) curve in 1D space.
/// </summary>
public sealed partial class NURBS1D
{
    /// <summary>
    ...
    /// <returns>A new instance of the <see cref="NURBS1D"/> class representing a uniform B-Spline.</returns>
    public static NURBS1D GetUniformBSpline(float[] points, int degree = 3, bool open = true)
    {
        int ptCount = points.Length;
        float[] knots = SplineUtils.GenerateUniformKnots(degree, ptCount, open);
        return new NURBS1D(points, knots, null, degree);
    }
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NURBS1D"/> class.
    /// </summary>
    ...
    public NURBS1D(float[] points, float[] knots, float[]? weights, int degree = 3)
    {
    ...
    public float GetPointByKnotValue(float t)
    {
