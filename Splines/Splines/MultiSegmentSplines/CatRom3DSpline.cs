...
using Splines.Splines.NonUniformSplineSegments;
using Splines.Unity;

namespace Splines.Splines.MultiSegmentSplines;

/// <summary>
/// Represents a Catmull-Rom spline in 3D space, with support for different endpoint modes and alpha values.
/// </summary>
[Serializable]
public sealed partial class CatRom3DSpline
{
    /// <summary>
    ...
    /// <inheritdoc cref="NUCatRomCubic3D.Alpha"/>
    public float Alpha
    {
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        EndpointMode endpointMode = EndpointMode.None)
    {
        ...
        Nodes = points.Zip(knots, (p, k) => new Node3 { Position = p, Knot = k }).ToList();
        _isDirty = true;
        ...
        EndpointMode endpointMode = EndpointMode.None)
    {
        ...
        Nodes = points.Select(p => new Node3 { Position = p }).ToList();
        _alpha = alpha;
        _isDirty = true;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetSecondDerivative(float u)
    {
        u = ReadyAndClampU(u);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetThirdDerivative(float u)
    {
    ...
    public void ReadyKnotsAndCoefficients()
    {
        if (_isDirty)
        {
            ...
            for (int i = 0; i < ControlPointCount - 1; i++)
            {
                ...
                Vector3Matrix4x1 pts = new(GetControlPoint(i - 1), n.Position, GetControlPoint(i + 1), GetControlPoint(i + 2));
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetPoint(int curve, float t) => GetPointInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the derivative with respect to <c>u</c> along a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    /// <param name="t">The fraction along this segment from 0 to 1</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetDerivative(int curve, float t) => GetDerivativeInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the second derivative with respect to <c>u</c> along a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    /// <param name="t">The fraction along this segment from 0 to 1</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetSecondDerivative(int curve, float t) => GetSecondDerivativeInternal(curve, RangeCheckAndGetU(curve, t));

    /// <summary>Returns the third derivative with respect to <c>u</c> of a curve by index</summary>
    /// <param name="curve">The index of the curve to sample</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Vector3 GetThirdDerivative(int curve) => GetThirdDerivativeInternal(curve);
    ...
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Vector3 GetPointInternal(int curve, float u) => Nodes[curve].EvalPoint(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Vector3 GetDerivativeInternal(int curve, float u) => Nodes[curve].EvalDerivative(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Vector3 GetSecondDerivativeInternal(int curve, float u) => Nodes[curve].EvalSecondDerivative(u);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Vector3 GetThirdDerivativeInternal(int curve) => Nodes[curve].EvalThirdDerivative();
