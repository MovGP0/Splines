...
public static class OutlierTerminator3D
{
    [Pure]
    public static IEnumerable<Vector3> EliminateOutliers(List<Vector3> points, float deviationThresholdFactor = 0.3f)
    {
        if (points.Count < 3)
        {
            return points;
        ...
        var infos = new OutlierPointInfo3D[points.Count];
        for (var i = 0; i < infos.Length; i++)
        {
            infos[i] = new OutlierPointInfo3D(points[i]);
        }
    ...
    private static void CalculateAccelerations(OutlierPointInfo3D[] infos)
    {
