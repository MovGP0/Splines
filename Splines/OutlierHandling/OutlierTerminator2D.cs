...
public static class OutlierTerminator2D
{
    [Pure]
    public static IEnumerable<Vector2> EliminateOutliers(List<Vector2> points, float deviationThresholdFactor = 0.3f)
    {
        if (points.Count < 3)
        {
            return points;
        ...
        var infos = new OutlierPointInfo2D[points.Count];
        for (var i = 0; i < infos.Length; i++)
        {
            infos[i] = new OutlierPointInfo2D(points[i]);
        }
    ...
    private static void CalculateAccelerations(OutlierPointInfo2D[] infos)
    {
